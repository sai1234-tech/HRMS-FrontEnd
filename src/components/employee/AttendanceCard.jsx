...
function AttendanceCard({ attendance, onClockIn, onClockOut }) {
  ...
  return <section className="attendance-card panel"><div className="attendance-card-content"><span className="card-label">Today's attendance</span><h2 className={attendance?.checkOut ? "attendance-completed" : ""}>{displayStatus}</h2><p className={attendance?.checkOut ? "attendance-complete-message" : ""}>{checkedIn ? "Your workday is in progress." : attendance?.checkOut ? "Your workday is complete." : "Start your workday when you are ready."}</p><div className="timer-display" aria-live="polite"><strong className="work-timer">{formatDuration(elapsedSeconds)}</strong><span className="timer-caption">Working time</span></div></div><div className="card-actions"><button type="button" className="primary-button" onClick={onClockIn} disabled={Boolean(attendance?.checkIn)}>Clock in</button><button type="button" className="secondary-button" onClick={onClockOut} disabled={!checkedIn}>Clock out</button></div></section>;
}
